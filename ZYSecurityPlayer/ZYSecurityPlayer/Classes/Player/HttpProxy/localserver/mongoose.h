...
#define MG_MGR_EV_MGR 0  // select() IO
/*
...
#include "LogHelper.h"
#define __DBG2  TVHttpProxyLOG_DEBUG
#define __DBG(x)                \
do {                          \
printf("%-20s ", __func__); \
__DBG2 x;                   \
...
//后续去掉日志输出，需要注释下面这个宏定义
//#define MG_ENABLE_DEBUG
...
#define MG_EV_ERROR 6  /* Connection is error. int* socket_errno */
...
struct mg_mgr {
  ...
  void *mgr_data;           /* Implementation-specific event manager's data. */
    int  hard_error_flag;        /* if occur hard error, set this flag */
};
